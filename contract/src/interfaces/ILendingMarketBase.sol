...
interface ILendingMarketBase {
    ...
    /// @notice UZR liquidation address
    /// @dev Only has power to liquidate UZR positions after the maturity
    function uzrLiquidator() external view returns (address);
    ...
    /// @notice Sets `newUslLiquidator` as the UZR liquidation address
    function setUzrLiquidator(address newUzrLiquidator) external;
